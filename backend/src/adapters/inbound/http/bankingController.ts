...
import { applyBanked as applyBankedFn } from '../../../core/application/applyBanked';
...
bankingRouter.get('/records', async (req, res, next) => {
  try {
    ...
    if (!shipId || Number.isNaN(year)) return res.status(400).json({ error: 'shipId and year required' });
...
bankingRouter.post('/bank', async (req, res, next) => {
  try {
    ...
    if (!shipId || !year || !Number.isFinite(amount)) return res.status(400).json({ error: 'shipId, year, amount required' });

    // get latest compliance snapshot
    const snapshot = await complianceRepo.getCompliance(shipId, year);
    if (!snapshot) return res.status(400).json({ error: 'No compliance snapshot, run /compliance/cb first' });

    const currentCb = snapshot.cbBeforeTonnes;
    if (amount <= 0) return res.status(400).json({ error: 'amount must be positive' });

    if (amount > currentCb) return res.status(400).json({ error: 'Amount exceeds available CB' });
...
bankingRouter.post('/apply', async (req, res, next) => {
  try {
    ...
    const banked = await bankingRepo.getBankedAmount(shipId, year);
    if (amount > banked) return res.status(400).json({ error: 'Amount exceeds available banked' });
